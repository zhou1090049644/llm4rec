...
class Trainer(object):
    ...
    def __init__(self, args, model, data_num,device):
        # 一个 epoch 有多少个 batch”当作 data_num 传进来
        self.args = args
        self.device = device
        self.model = model
        ...
        self.summary_writer =self._build_summary_writer()
        # self.model = self.model.to(self.device)


    def _build_summary_writer(self):
        log_dir = os.environ.get("RQVAE_TF_EVENTS_PATH", "./logs")
        ensure_dir(log_dir)
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        for batch_idx, data in enumerate(iter_data):
            ...
            self.summary_writer.add_scalar('Step/loss_total', loss.item(), epoch_idx * len(iter_data) + batch_idx)
            self.summary_writer.add_scalar('Step/loss_recon', loss_recon.item(), epoch_idx * len(iter_data) + batch_idx)
    ...
    def _valid_epoch(self, valid_data):
        ...
        for batch_idx, data in enumerate(iter_data):
            ...
            if used_code_sets is None:
                used_code_sets = [set() for _ in range(indices.shape[-1])]
            # 不会做decoder重构，只拿encoder + residual quantizer得到每个样本的多层离散码[123, 88, 907]
            for index in indices:
                code = "-".join([str(int(_)) for _ in index])
                indices_set.add(code)
                for layer_idx, code_idx in enumerate(index):
                    used_code_sets[layer_idx].add(int(code_idx))

        # 验证指标为 码碰撞率
        # 如果每个样本码都不同，碰撞率接近 0
        # 如果很多样本映射到同一组 code，碰撞率会上升
        collision_rate = (num_sample - len(list(indices_set)))/num_sample

        collision_rate = (num_sample - len(indices_set)) / num_sample
        if used_code_sets is not None:
            num_emb_list = self.args.num_emb_list[:len(used_code_sets)]
            combo_total = 1
            for num_emb in num_emb_list:
                combo_total *= num_emb
            for layer_idx, (used_codes, num_emb) in enumerate(zip(used_code_sets, num_emb_list)):
            ...
            print(
                f"Combination utilization: {len(indices_set)}/{combo_total} "
                f"({len(indices_set) / combo_total:.6%})"
            )
    ...
    def fit(self, data):
        ...
        for epoch_idx in range(self.epochs):
            ...
            self.summary_writer.add_scalar('Epoch/loss_train', train_loss, epoch_idx)
            self.summary_writer.add_scalar('Epoch/loss_recon', train_recon_loss, epoch_idx )
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                self.summary_writer.add_scalar('Epoch/collision_rate', collision_rate, epoch_idx)
